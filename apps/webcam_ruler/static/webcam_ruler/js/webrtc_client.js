...
function negotiate(signalingSocket, pc) {


    return pc.createOffer()
            .then( function(offer) { return pc.setLocalDescription(offer); } )
            .then( function() { waitForIceStatePromise(pc); } )
            .then( function() { sendOfferPromise(signalingSocket, pc.localDescription); } )
            .then( function() { waitForAnswerPromise(pc, signalingSocket); } )
            .then( function() { console.log('webrtc negotiation completed!') })
            .catch( function(e) {
                console.error(e);
                alert(e);
            });
...
function waitForIceStatePromise(pc) {
...
function sendOfferPromise(signalingSocket, offer) {

    offer.sdp = sdpFilterCodec('video', video_codec, offer.sdp);

    function sendOffer(resolve){
        offer_json = JSON.stringify({ sdp: offer.sdp, type: offer.type })
        if (signalingSocket.readyState === WebSocket.OPEN ) {
            signalingSocket.send(offer_json);
            resolve();
        } else { signalingSocket.addEventListener("open", (response) => {
            signalingSocket.removeEventListener("open", sendOffer);
            signalingSocket.send(offer_json);
            resolve();
        });}
    }
    return new Promise(sendOffer);
}

function waitForAnswerPromise(pc) {
    function waitForAnswer(resolve){
        signalingSocket.onmessage = (response) => {
            signalingSocket.onmessage = null;
            result = JSON.parse(response.data);
            return pc.setRemoteDescription(result);
        }
    }
    return new Promise(waitForAnswer);
}
