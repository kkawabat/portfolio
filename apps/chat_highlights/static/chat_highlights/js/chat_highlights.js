...
function genChart(container, data) {
  ...
  const color =  i => d3.schemeBlues[Math.max(3, bands)][i + Math.max(0, 3 - bands)];

  // Create the container and append a canvas for each series.
  const div = d3.select('body').selectAll('div').style("position", "relative");
